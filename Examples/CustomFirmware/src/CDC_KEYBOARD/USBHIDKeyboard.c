...
uint8_t Keyboard_press_non_assci(uint8_t k) {
	uint8_t i;
...
uint8_t Keyboard_release_non_ascii(uint8_t k) {
	uint8_t i;
...
uint8_t Keyboard_press_ascci(uint8_t k) {
	...
	return Keyboard_press_non_assci(k);
}

uint8_t Keyboard_write_non_ascii(uint8_t c){
	uint8_t p = Keyboard_press_non_assci(c);  // Keydown
	Keyboard_release_non_ascii(c);            // Keyup
	return p;              // just return the result of press() since release() almost always returns 1
}

uint8_t Keyboard_release_ascii(uint8_t k) {
	if (k >= 136) {			// it's a non-printing key (not a modifier)
		k = k - 136;
	} else if (k >= 128) {	// it's a modifier key
		HIDKey[0] &= ~(1<<(k-128));
	...
	return Keyboard_release_non_ascii(k);
}
...
uint8_t Keyboard_write_ascii(uint8_t c){
	uint8_t p = Keyboard_press_ascci(c);  // Keydown
	Keyboard_release_ascii(c);            // Keyup
	return p;              // just return the result of press() since release() almost always returns 1
}

uint8_t Keyboard_write_string(const char * str){
	while(*str!='\0')
    {
        Keyboard_write_ascii(*str++);
    }
}
